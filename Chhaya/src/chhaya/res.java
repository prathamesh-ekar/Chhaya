...
public class res extends JFrame
{
	...
	public void n1(int it[],int coun[],int cu)
	{
		...
		try
			...
			Connection c = DriverManager.getConnection("jdbc:mysql://localhost:3306/PhotoLab","root","root");
				...
				for(in=0;in<cont;in++)
				{
				ResultSet rs = stm.executeQuery("select * from stock where p_id="+ite[in]);
				while(rs.next())
				{	
					t=1;	
					j=0;
				...
				if(t==0)
				{
					jp.showMessageDialog(this,"Sorry, No Such Record exists","INFORMATION",jp.ERROR_MESSAGE);
					t=0;
				}
				}
		        	c.close();
