...
public class GenerateBill extends JFrame implements ActionListener
{
	...
	public GenerateBill()
	{
		...
		c.add(ge);
			...
			ResultSet rs = stm.executeQuery("select max(bill_id) from bill");
			t=0;
			while(rs.next())
			{
				t++;
				i=rs.getInt("max(bill_id)");
				i=i+1;
			}
			if(t==0)
			bid.setText("1");
	...
	{	
		...
		if(source==add)
		{
			...
			{	ResultSet rs1 = stm.executeQuery("select * from bill where bill_id="+bid.getText()+" and itid="+id.getText());
				while(rs1.next())
				{	
					t=1;
				}
